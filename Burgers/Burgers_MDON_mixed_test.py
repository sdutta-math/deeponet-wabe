...
#os.environ["TF_USE_LEGACY_KERAS"] = "1"  ## Needed to use Keras 2 with TF 2.16+
...
import settings_config_1 as sett
...
import modified_don_mixed as don
...
if case == 'Predict':
    ...
    model_path = model_dir+'_burgers1d_mdon'+timestamp_don+model_suffix
...
if case == 'Train':
    ...
    #              callbacks=[keras.callbacks.TensorBoard("tb/train_logs"+str(i)),reduce_lr])#,early_stop])
    model.fit(dataset, validation_data=val_dataset, epochs= train_epochs,
...
# Saving to CSV
csv_filename = model_suffix + 'model_history.csv'
...
#plt.savefig(model_suffix + "_learning rate")
