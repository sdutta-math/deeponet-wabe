...
class don_nn(keras.models.Model):
    def __init__(self, l_factor, latent_dim, branch_input_shape, b_number_layers, b_neurons_layer, b_actf, b_init, b_regularizer, b_encoder_layers, b_encoder_neurons, b_encoder_actf, b_encoder_init, b_encoder_regularizer, trunk_input_shape, t_number_layers, t_neurons_layer, t_actf, t_init, t_regularizer, t_encoder_layers, t_encoder_neurons, t_encoder_actf, t_encoder_init, t_encoder_regularizer, dropout=False, dropout_rate=0.1, **kwargs):
        ...
        #keras.utils.plot_model(self.branch_encoder, to_file=branch_encoder_img_file, show_shapes=True)

        trunk_encoder_img_file = 'trunk_encoder.png'
        #keras.utils.plot_model(self.trunk_encoder, to_file=trunk_encoder_img_file, show_shapes=True)
        ...
        #keras.utils.plot_model(self.trunk, to_file=trunk_img_file, show_shapes=True)
    ...
    def Branch_Encoder(self, input_shape, neurons_layer, encoder_layers, encoder_neurons, actf, init, regularizer):
        ...
        for e in range(encoder_layers):
            encoder = keras.layers.Dense(encoder_neurons, activation=actf, kernel_initializer=init, kernel_regularizer=regularizer)(encoder)
            if self.dropout==True: 
                encoder = keras.layers.Dropout(self.dropout_rate)(encoder)
        output_encoder = keras.layers.Dense(neurons_layer, activation=actf, kernel_initializer=init, kernel_regularizer=regularizer)(encoder)
        model = keras.Model(input_layer,output_encoder)
        return model
    ...
    def MLP_Branch(self, branch_input_shape, trunk_input_shape, output_shape, number_layers, neurons_layer, actf, init, regularizer):
        branch_input_layer = keras.layers.Input(shape=(branch_input_shape,),name='branch_input')
        trunk_input_layer = keras.layers.Input(shape=(trunk_input_shape,),name='trunk_input')
        x = branch_input_layer
        for i in range(number_layers):
            x = keras.layers.Dense(neurons_layer,activation=actf,kernel_initializer=init,kernel_regularizer=regularizer,name='branch_hidden'+str(i))(x)
            ones = tf.ones_like(x)
            x1 = keras.layers.Subtract(name='subtract_branch'+str(i))([ones,x])
            x2 = keras.layers.Multiply(name='multiply_branch_encoder'+str(i))([x,self.branch_encoder(branch_input_layer)])
            x3 = keras.layers.Multiply(name='multiply_trunk_encoder'+str(i))([x1,self.trunk_encoder(trunk_input_layer)])  
            x = keras.layers.Add(name='add_product'+str(i))([x2,x3])
            if self.dropout==True: 
                x = keras.layers.Dropout(self.dropout_rate)(x)
        output_layer = keras.layers.Dense(output_shape,kernel_initializer=init,kernel_regularizer=regularizer,name='output_branch')(x)
        model = keras.Model([branch_input_layer,trunk_input_layer],output_layer)
        return model
    ...
    def MLP_Trunk(self, branch_input_shape, trunk_input_shape, output_shape, number_layers, neurons_layer, actf, init, regularizer):
        ...
        for i in range(number_layers):
            ...
            if self.dropout==True: 
                x = keras.layers.Dropout(self.dropout_rate)(x)
        output_layer = keras.layers.Dense(output_shape, activation=actf, kernel_initializer=init,kernel_regularizer=regularizer,name='output_trunk')(x)
